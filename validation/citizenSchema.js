...
const citizenSchema = Joi.object({
  ID_CARD: Joi.string().length(13).pattern(/^\d+$/).required(), // 13-digit numeric string
  ...
  village_number: Joi.string().max(5).pattern(/^\d+$/).required(), // Must be a numeric string
  district: Joi.string().max(100).required(),
  soi: Joi.string().max(10).allow(null, ""), // Allow null or empty string
