...
router.get("/complete_land", (req, res) => {
  connection.query(
    `
        SELECT prefix_name, first_name, last_name, p.ID_CARD AS "ID", land_status_name, 
        l.id_land, current_soi
FROM land l
JOIN people p ON l.id_card = p.ID_CARD
JOIN prefix pr ON p.prefix_id = pr.prefix_id
JOIN land_status ls ON l.current_land_status = ls.ID_land_status;
`,
    (err, results) => {
      if (err) {
...
router.get("/sois", (req, res) => {
  connection.query("SELECT id_alley FROM alley", (err, results) => {
    if (err) {
      console.error("Error executing query: " + err.stack);
      return res.status(500).send("Database query error");
    }
...
router.get("/land_status", (req, res) => {
  connection.query(
    "SELECT * FROM land_status ORDER BY ID_land_status",
    (err, results) => {
      if (err) {
        console.error("Error executing query: " + err.stack);
        return res.status(500).send("Database query error");
